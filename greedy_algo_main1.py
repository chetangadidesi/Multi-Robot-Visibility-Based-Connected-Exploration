...
if __name__ == "__main__":
    ...
    frontier_cache = {} # <-- CACHING IS BACK
    ...
    print("--- Simulation Start (FINAL: Chained-Auction + A* Paths + Caching) ---")
    ...
    while len(stationary_drones) < num_stationary_goal:
        ...
        if pending_map_update_view is not None:
            ...
            keys_to_delete = []
            clean_winner_view = pending_map_update_view.buffer(TOLERANCE)
            for key, value in frontier_cache.items():
                if clean_winner_view.contains(Point(value['pos'])):
                    keys_to_delete.append(key)
            print(f"         - Invalidated {len(keys_to_delete)} cached frontiers.")
            for key in keys_to_delete:
                del frontier_cache[key]
        ...
        for frontier in task_list_raw:
            ...
            for s_drone in stationary_drones:
                if s_drone.vis_poly is None: 
                    print(f"         ! ERROR: Stationary D{s_drone.id} has no vis_poly!")
                    continue
        ...
        print(f"--- 1. SENSE: Found {len(all_current_frontiers)} total frontiers.")
        update_plot(ax, polygon, drone_fleet, shared_map, f"Round {round_num}: Found {len(all_current_frontiers)} Frontiers", frontiers=frontiers_geom)
        time.sleep(0.1)

        # --- 2. EVALUATE (with Caching) ---
        print("--- 2. EVALUATE: Sorting new vs. known frontiers.")
        ...
        for task in all_current_frontiers:
            ...
            if key in frontier_cache:
                ...
                cached_data['parent_drone'] = task['parent_drone'] # Update parent
        ...
        print(f"         - Sorting complete: {len(cached_tasks_fresh)} fresh, {len(tasks_for_auction_stale)} stale, {len(tasks_for_auction_new)} new.")

        newly_evaluated_tasks = []
        re_evaluated_tasks = []
        all_drones_in_field_this_round = []
        
        # --- AUCTION 1: Evaluate all *NEW* frontiers ---
        print(f"--- 2a. AUCTION 1: Evaluating {len(tasks_for_auction_new)} new frontiers. ---")
        ...
        # --- 2b. STALENESS CHECK ---
        print("--- 2b. STALENESS CHECK: Comparing new vs. cached gains. ---")
        max_new_gain = -1.0
        if newly_evaluated_tasks:
            max_new_gain = max(task['gain'] for task in newly_evaluated_tasks)

        max_cached_gain = -1.0
        all_cached_data = cached_tasks_fresh + cached_tasks_stale_data
        if all_cached_data:
            max_cached_gain = max(task.get('gain', -1.0) for task in all_cached_data)
        
        if max_cached_gain > max_new_gain:
            print(f"         - STALE GAIN DETECTED! Max cached ({max_cached_gain:.1f}) > Max new ({max_new_gain:.1f}).")
            
            # --- SEQUENTIAL RE-EVALUATION LOGIC ---
            ...
            print(f"         - Filtering stale list: {len(stale_candidates_for_re_eval)} tasks to re-check sequentially, {len(trusted_stale_data)} trusted low-gain tasks.")

            # --- 2c. SEQUENTIAL RE-EVALUATION ---
            print("--- 2c. SEQUENTIAL RE-EVALUATION: Checking high-gain stale tasks... ---")

            best_task_so_far = None
            current_champion_gain = max_new_gain
            if newly_evaluated_tasks:
                   best_task_so_far = max(newly_evaluated_tasks, key=lambda x: x.get('gain', -1.0))
            ...
            for i, (task_to_check, stale_data) in enumerate(stale_candidates_for_re_eval):
                ...
                if cached_gain <= current_champion_gain:
                    print(f"         - Remaining {len(stale_candidates_for_re_eval) - i} tasks have cached gain <= {current_champion_gain:.1f}. Stopping sequential check.")
                    break 
                    
                mobile_drones = [d for d in drone_fleet if d.status == 'available']
                if not mobile_drones:
                    print("         - No mobile drones available to re-evaluate. Stopping sequential check.")
                    break
                ...
                print(f"         - Re-evaluating task at {np.round(task_to_check['target_pos'], 1)} (stale gain: {cached_gain:.1f}) with D{drone.id}...")
                ...
                animate_drones_parallel(
                    animation_data_list_out, f"D{drone.id} re-evaluating task", 
                    path_key='out_path_points',
                    ax=ax, polygon=polygon, drone_fleet=drone_fleet, 
                    shared_map=shared_map, frontiers=frontiers_geom, round_num=round_num,
                    evaluated_tasks=(newly_evaluated_tasks + re_evaluated_tasks),
                    winning_task=best_task_so_far 
                )
                ...
                print(f"         - D{drone.id} reports *new* gain: {new_updated_gain:.2f} (was {cached_gain:.1f})")

                task_result = {
                    'pos': best_point, 
                    'gain': new_updated_gain, 
                    'parent_drone': task_to_check['parent_drone'],
                    'map_area_when_cached': current_map_area 
                }
                ...
                if key:
                    frontier_cache[key] = task_result # Update cache

                if new_updated_gain > current_champion_gain:
                    print(f"         - NEW CHAMPION FOUND! Gain {new_updated_gain:.1f} > {current_champion_gain:.1f}")
                ...
                if (i + 1) < len(stale_candidates_for_re_eval):
                    ...
                    if current_champion_gain >= next_task_cached_gain:
                        print(f"         - Champion gain {current_champion_gain:.1f} >= next task's cached gain {next_task_cached_gain:.1f}.")
                        print(f"         - ** OPTIMIZATION: Stopping sequential re-evaluation early. **")
                        
                        drone.pos = waypoints[-1] 
                        drone.status = 'available' 
                        drone.current_task_target_pos = task_to_check['target_pos']
                        break 
            ...
            # --- END OF SEQUENTIAL RE-EVALUATION ---

            num_checked = len(re_evaluated_tasks)
            untested_stale_data = [data for task, data in stale_candidates_for_re_eval[num_checked:]]
            
            print(f"         - Sequential check complete. {len(re_evaluated_tasks)} tasks re-checked.")
            print(f"         - {len(untested_stale_data)} high-gain tasks were skipped and will use cached data.")
            ...
            # --- NO STALE GAIN ---
            print(f"         - Stale gain check passed. Max new ({max_new_gain:.1f}) >= Max cached ({max_cached_gain:.1f}).")
    ...
    print(f"Algorithm:           Chained-Auction + A* Paths (Robust Nav + Caching)")
