...
# ==============================================================================
# TRIANGULAR EXPANSION - EXACT PREDICATES
# (Robust geometric checks from your code)
# ==============================================================================
...
def get_visibility_from_triangulation(
    ...
    This function replaces the old 'cast_rays_with_holes'.
    """
    qx, qy = viewpoint
    
    # 1. Run the fast triangular expansion algorithm
    visibility_segments = triangular_expansion(delaunator_data, qx, qy, obstructs_func)
    
    if not visibility_segments:
        return Polygon()  # Return an empty polygon if viewpoint is outside or sees nothing

    # 2. Convert the visible segments into a list of triangular Polygon "wedges"
    visible_triangles = []
    for seg in visibility_segments:
        # A segment is (slx, sly, srx, sry)
    ...
    # 3. Union all the small triangles into one large visibility polygon
    #    This is the same as the final output from the old ray-casting method.
    return unary_union(visible_triangles).simplify(1e-9)


# ==============================================================================
# FRONTIER CALCULATION (from Drone code - This is still needed)
# ==============================================================================
...
# ==============================================================================
# DRONE SIMULATION CLASSES AND FUNCTIONS (from Drone code)
# ==============================================================================
class Drone:
...
def update_plot(ax, polygon, drone_fleet, shared_map, title_text, frontiers=None, evaluated_tasks=None, winning_task=None):
    ...
    # Plot the base environment polygon (zorder=0)
    ox, oy = polygon.exterior.xy
    ax.plot(ox, oy, 'k-', linewidth=2, zorder=0) # Base boundary line
    ax.fill(ox, oy, color='lightgray', zorder=0) # Fill background with light gray
    ...
    for i, poly in enumerate(polygons_to_plot):
        ...
        # Plot the exterior (the yellow part) at zorder=1
        vx, vy = poly.exterior.xy
        ax.fill(vx, vy, color='gold', alpha=0.5, label='Total Coverage' if i == 0 else '_nolegend_', zorder=1)
        
        # Plot the interiors (punching out the holes) at zorder=2
        for interior in poly.interiors:
            ix, iy = interior.xy
            ax.fill(ix, iy, color='lightgray', zorder=2) # zorder=2 to be on top of yellow

    # zorder 3: Environment hole (obstacle) fills
    for interior in polygon.interiors:
        hx, hy = interior.xy
        ax.fill(hx, hy, color='dimgray', zorder=3) # zorder 3
        
    # zorder 4: Environment boundaries (exterior AND holes)
    # This draws all black lines on top of all fills
    ax.plot(ox, oy, 'k-', linewidth=1, zorder=4) # Base boundary line
    for interior in polygon.interiors:
        hx, hy = interior.xy
        ax.plot(hx, hy, 'k-', linewidth=1, zorder=4) # Hole boundary line
        
    # --- END CORRECTED PLOTTING ---


    # Plot network links (zorder=3)
    ...
    # Plot frontiers (cyan, zorder=4)
    if frontiers:
        if frontiers.geom_type == 'MultiLineString':
            for i, frontier in enumerate(frontiers.geoms):
                x, y = frontier.xy
                ax.plot(x, y, 'c--', linewidth=2, label='Frontiers' if i == 0 else '_nolegend_', zorder=4)
        elif frontiers.geom_type == 'LineString':
             x, y = frontiers.xy
             ax.plot(x, y, 'c--', linewidth=2, label='Frontiers', zorder=4)

    # Plot evaluated tasks (gain text, zorder=5)
    if evaluated_tasks:
        for task in evaluated_tasks:
            ax.text(task['pos'][0], task['pos'][1] + 0.3, f"{task['gain']:.1f}", color='blue', fontsize=9, ha='center', fontweight='bold', zorder=5)
    
    # Plot winning task frontier (gold solid, zorder=6)
    if winning_task and frontiers:
        winning_point = Point(winning_task['pos'])
        all_frontiers = list(frontiers.geoms) if frontiers.geom_type == 'MultiLineString' else [frontiers]
        for frontier in all_frontiers:
            if frontier.interpolate(0.5, normalized=True).equals_exact(winning_point, 1e-6):
                x, y = frontier.xy
                ax.plot(x, y, color='gold', linestyle='-', linewidth=4, zorder=6, label='Winning Frontier')
                break
                
    # Plot drones (zorder=7)
    for drone in drone_fleet:
        ...
        ax.plot(drone.pos[0], drone.pos[1], marker, color=color, markersize=size, label=label, zorder=7)
        ax.text(drone.pos[0] + 0.2, drone.pos[1] + 0.2, f'D{drone.id}', color=color, fontsize=10, fontweight='bold', zorder=8)
...
if __name__ == "__main__":
    ...
    # 1. Define the Environment Polygon (from Drone code)
    ...
    # 2. ================== CORRECTED SECTION ==================
    # Convert Shapely Polygon to `triangle` input format
    
    # --- Exterior ---
    # Get unique exterior vertices (slicing [:-1] removes the duplicate closing point)
    exterior_coords = list(polygon.exterior.coords[:-1]) 
    vertices = list(exterior_coords)
    ext_len = len(vertices)
    # Create looped segments for the exterior
    segments = [[i, (i + 1) % ext_len] for i in range(ext_len)] 
    
    # --- Interiors ---
    holes_list = []
    # The starting index for the *first* hole's vertices
    start_idx = len(vertices) 
    
    for interior in polygon.interiors:
        # Get unique hole vertices (slicing [:-1])
        hole_coords = list(interior.coords[:-1]) 
        if not hole_coords: continue # Skip if hole is empty
        hole_len = len(hole_coords)
        
        # Add these vertices to the main list
        vertices.extend(hole_coords) 
        
        # Create looped segments for *this* hole, offset by start_idx
        segments.extend([[start_idx + i, start_idx + (i + 1) % hole_len] for i in range(hole_len)])
        
        # Add a point *inside* the hole for the 'holes' list
        # We use the original 'interior' object to make a valid polygon
        holes_list.append(list(Polygon(interior).centroid.coords)[0])
        
        # Update the start_idx for the *next* hole
        start_idx += hole_len

    # --- Build the final 'scene' dict ---
    scene = {
        'vertices': np.array(vertices),
        'segments': np.array(segments),
    }
    if holes_list:
        # Only add the 'holes' key if we have them
        scene['holes'] = np.array(holes_list)
        
    # 3. ================== END CORRECTED SECTION ==================

    # 4. NEW: Run Triangulation ONCE
    # 'p' preserves segments (so they are walls), 'A' assigns attributes
    B = tr.triangulate(scene, 'pA') # This should now work
    
    # 5. NEW: Build the data structures for the TE algorithm
    ...
    # 6. Initialize Drone Fleet (from Drone code)
    ...
    # 7. Run Main Simulation Loop (from Drone code)
    round_num = 1
    while len(stationary_drones) < num_stationary_goal:
        ...
        task_list = [frontier.interpolate(0.5, normalized=True).coords[0] for frontier in tasks_geom if frontier.length > 0.01] # Added safety check
        ...
        while task_list:
            ...
            for assignment in assignments:
                ...
                view = get_visibility_from_triangulation(
                    drone.pos, delaunator_like_data, obstructs
                )
        ...
        if not broadcast_channel: 
            print("   - No valid tasks found. Ending simulation.")
            break
        ...
        if not mobile_drones:
             print("   - No mobile drones left to place. Ending simulation.")
             break
    ...
    # --- NEW: Calculate and display final coverage ---
    ...
    update_plot(ax, polygon, drone_fleet, shared_map, f"Coverage Complete! ({len(stationary_drones)} Drones Placed) with {coverage_percentage:.2f}% Coverage")
    plt.ioff()
    plt.show()